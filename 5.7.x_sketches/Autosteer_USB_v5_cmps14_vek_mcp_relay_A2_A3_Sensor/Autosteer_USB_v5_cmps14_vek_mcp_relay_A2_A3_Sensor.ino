...
#define Relay_Type 1 // set to 1 if using mcp23017 port expander
#define NUMBER_OF_MCPS 2    //0 1 or 2

/////////////////////////////////////////////

#include "Adafruit_MCP23017.h"
Adafruit_MCP23017 mcps[NUMBER_OF_MCPS];
uint32_t mcplastTime = 0;
...
#include <EEPROM.h>
#include "ADS1X15.h"
ADS1115 ADS(0x48);  // Use this for the 16-bit version ADS1115
...
//  adc.setGain(ADS1115_REG_CONFIG_PGA_6_144V);
  ...
  {
    // initialize mcp23017
    for (int n = 0; n < NUMBER_OF_MCPS; n++) {
      mcps[n].begin(n);
      Wire.beginTransmission(0x20 + n);
      error = Wire.endTransmission();

      if (error == 0) {
        Serial.print("MCP found at :");
        Serial.println(0x20 + n, HEX);
      }
      else if (error == 2) {
        Serial.print("MCP ERROR NOT found at :");
        Serial.println(0x20 + n, HEX);
      }
      mcps[n].pinMode(8, OUTPUT);
      mcps[n].pinMode(9, OUTPUT);
      mcps[n].pinMode(10, OUTPUT);
      mcps[n].pinMode(11, OUTPUT);
      mcps[n].pinMode(12, OUTPUT);
      mcps[n].pinMode(13, OUTPUT);
      mcps[n].pinMode(14, OUTPUT);
      mcps[n].pinMode(15, OUTPUT);
      delay(100);
    }
    for (int n = 0; n < NUMBER_OF_MCPS; n++) {
      mcps[n].digitalWrite(8, LOW);
      mcps[n].digitalWrite(9, LOW);
      mcps[n].digitalWrite(10, LOW);
      mcps[n].digitalWrite(11, LOW);
      mcps[n].digitalWrite(12, LOW);
      mcps[n].digitalWrite(13, LOW);
      mcps[n].digitalWrite(14, LOW);
      mcps[n].digitalWrite(15, LOW);
...
void loop()
{
  ...
  if (currentTime - mcplastTime >= 100 && NUMBER_OF_MCPS > 0)
  {
    mcplastTime = currentTime;
#if Relay_Type == 1
  ...
  if (currentTime - lastTime >= LOOP_TIME)
  {
   ...
   // Pressure sensor?
    ...
    {
      int16_t analogValue = ADS.readADC(3);
      sensorSample = (analogValue >> 1);
      sensorSample /= 13;
    ...
    {
      int16_t analogValue = ADS.readADC(2);
      sensorSample = (analogValue >> 1);
      sensorSample /= 13;
    ...
    if (steerConfig.SingleInputWAS)   //Single Input ADS
    {
      ...
      steeringPosition = ADS.readADC(0);
